...
def nmetrics(a):
    ...
    return [float(uciqe),float(uiqm), float(uiconm), float(uism), float(uicm)]
...
def single_metric():
    folder_path = r'D:\VIT\Projects\Underwater-Image-Enhancement-VCSE_UResnet\UResnet\evaluate\original'
    files = os.listdir(folder_path)
    l=[]
    for file in files:
    ...
    # Calculate the average for each position across all inner lists
    average_list = [sum(values) / len(values) for values in zip(*l)]
    print(average_list)  
