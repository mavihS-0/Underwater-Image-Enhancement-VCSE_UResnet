...
def main():
    ...
    reference_path = './to_compare'
    result_files = os.listdir(result_path)
    sumpsnr, sumssim, sumuiqm, sumuciqe, N = 0., 0., 0., 0., 0

    for img_file in result_files:
        if img_file.endswith('-output.png'):
            img_id = img_file.split('-output.png')[0].replace('processed_', 'img_')
            reference_file = f'{img_id}.png'
            if os.path.exists(os.path.join(reference_path, reference_file)):
                corrected = io.imread(os.path.join(result_path, img_file))
                reference = io.imread(os.path.join(reference_path, reference_file))
                ...
                with open(os.path.join(result_path, 'metrics.txt'), 'a') as f:
                    f.write(f'{img_id}: psnr={psnr} ssim={ssim} uiqm={uiqm} uciqe={uciqe}\n')

    mpsnr, mssim, muiqm, muciqe = sumpsnr / N, sumssim / N, sumuiqm / N, sumuciqe / N
    with open(os.path.join(result_path, 'metrics.txt'), 'a') as f:
        f.write(f'Average: psnr={mpsnr} ssim={mssim} uiqm={muiqm} uciqe={muciqe}\n')
